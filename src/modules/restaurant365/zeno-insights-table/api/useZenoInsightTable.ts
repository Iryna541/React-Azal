...
export type GetZenoInsightTableResponse = {
  insights_data: Array<ZenoInsightRow>;
  stores: Array<{
    id: number;
    name: string;
  }>;
...
export async function getZenoInsightTable(
  storeId?: string
): Promise<GetZenoInsightTableResponse> {
  if (!storeId) {
    return axios.get(`/analytics/getInsightsTable`).then((res) => res.data);
  }

  return axios
    .get(`/analytics/getInsightsTable?store_id=${storeId} `)
    .then((res) => res.data);
}
...
export function useZenoInsightTable({
  storeId,
  ...config
}: UseZenoInsightTableOptions = {}) {
  return useQuery({
