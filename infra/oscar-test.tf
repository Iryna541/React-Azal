resource "aws_elastic_beanstalk_environment" "eb_test_env" {
  name                = "${var.elasticbeanstalk_app_name}-Test"
...
resource "aws_cloudfront_distribution" "oscar_test_cf" {
  enabled         = true
  is_ipv6_enabled = true
  aliases         = ["oscar-test.azal.io"]

  origin {
    domain_name = aws_elastic_beanstalk_environment.eb_test_env.cname
    origin_id   = "EBEnv"
